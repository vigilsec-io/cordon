...
import subprocess
import pytest
from valca.rules.deps import OsvScannerRule
...
def test_osv_tool_not_found_in_candidates_returns_empty(tmp_path):
    f = tmp_path / "go.sum"
    f.write_text("")
    with patch("valca.rules.deps._find_tool", return_value=None):
        assert rule.check(f) == []
...
def test_osv_one_vulnerability_parsed(tmp_path):
    f = tmp_path / "go.sum"
    f.write_text("")
    with patch("valca.rules.deps._find_tool", return_value="/opt/homebrew/bin/osv-scanner"):
